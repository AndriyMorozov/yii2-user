...
```
if (Yii::$app->session->has(\polucorus\user\controllers\AdminController::ORIGINAL_USER_SESSION_KEY))
    echo Html::a(
...
echo Nav::widget([
    ...
    'items' => [
        Yii::$app->session->has(\polucorus\user\controllers\AdminController::ORIGINAL_USER_SESSION_KEY) ?
